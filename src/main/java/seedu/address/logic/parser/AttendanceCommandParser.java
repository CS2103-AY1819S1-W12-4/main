...
public class AttendanceCommandParser implements Parser<AttendanceCommand> {
    ...
    public AttendanceCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap = ArgumentTokenizer.tokenize(args, PREFIX_ID, PREFIX_ATTENDANCE);

        if (!arePrefixesPresent(argMultimap, PREFIX_ID, PREFIX_ATTENDANCE) || argMultimap.getPreamble().isEmpty()) {
        ...
        AttendanceMark attendanceMark = ParserUtil.parseAttendanceMark(argMultimap.getValue(PREFIX_ATTENDANCE).get());
