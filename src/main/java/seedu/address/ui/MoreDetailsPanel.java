...
public class MoreDetailsPanel extends UiPart<Region> {
    ...
    public void display(int studentIndex, BaseEvent event) {
        ...
        setRowsAndColumns();
        int noOfUsedRows = showAssignments(student);
        showNote(student);
        showAttendance(noOfUsedRows);
    }

    /**
     * Shows the student's attendance for the lessons.
     * @param noOfUsedRows rows used already by Assignments.
     */
    //@@author zzsmao
    private void showAttendance(int noOfUsedRows) {
        int row = noOfUsedRows;
        row++;
        ...
        for (int i = 0; i < attendanceList.size(); i++) {
            ...
            label = createLabel(String.format("%d. %s", i + 1, attendance.getSession()), DEFAULT_STYLE);
            components.add(label, 0, row);

            label = createLabel(String.valueOf(attendance.getDate()), DEFAULT_STYLE);
            components.add(label, 1, row);

            label = createLabel(String.valueOf(attendance.getPresence()), DEFAULT_STYLE);
            components.add(label, 2, row);
    ...
    /**
     * Shows the given student's note.
     */
    //@@author spencertan96
